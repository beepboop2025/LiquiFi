...
import { ALERTS_FULL } from "./constants/alerts.js";
import { generateCashFlowHistory, generateHistoricalRates } from "./generators/history.js";
import { generateHourlyForecast } from "./generators/forecast.js";
import { generateRates } from "./generators/rates.js";
import { createBackendEngine } from "./engine/backendEngine.js";
import { validateDeploymentPlan } from "./engine/validation.js";
import { createBackendState, createRealtimePayment } from "./engine/backendState.js";
...
import { connectWebSocket, disconnectWebSocket, fetchForecast, fetchMonteCarlo, fetchCashFlowHistory } from "./services/api.js";
...
import TabCommandCenter from "./components/tabs/TabCommandCenter.jsx";
import TabAIEngine from "./components/tabs/TabAIEngine.jsx";
import TabOptimizer from "./components/tabs/TabOptimizer.jsx";
import TabExecution from "./components/tabs/TabExecution.jsx";
import TabRisk from "./components/tabs/TabRisk.jsx";
import TabInstruments from "./components/tabs/TabInstruments.jsx";
import TabAnalytics from "./components/tabs/TabAnalytics.jsx";
import TabRegulatory from "./components/tabs/TabRegulatory.jsx";
import TabBranches from "./components/tabs/TabBranches.jsx";
import TabSettings from "./components/tabs/TabSettings.jsx";

// Intervals in milliseconds
const FORECAST_REFRESH_MS = 30 * 60 * 1000; // 30 minutes
const CASHFLOW_REFRESH_MS = 60 * 60 * 1000; // 1 hour
const MC_REFRESH_MS = 30 * 60 * 1000;       // 30 minutes
...
export default function App() {
  ...
  const [clockData, setClockData] = useState(() => generateHourlyForecast());
  const [historicalRates] = useState(() => generateHistoricalRates());
  const [cashFlowHistory, setCashFlowHistory] = useState(() => generateCashFlowHistory());
  const [mcData, setMcData] = useState(null);
  const [time, setTime] = useState(new Date());
  const [backendConnected, setBackendConnected] = useState(false);
  const [forecastSource, setForecastSource] = useState("local"); // "local" | "lstm" | "synthetic"
  const [dataQuality, setDataQuality] = useState(null);
  const backendRatesRef = useRef(null);
  ...
  const syncBackendTelemetry = useCallback((opts = {}) => {
    ...
    setBackend((prev) => ({
      ...
      circuitOpenedAt: snapshot.metrics.circuitOpenedAt
        ? new Date(snapshot.metrics.circuitOpenedAt).toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata", hour12: false })
        : null,
      queueDepth,
      ...
      // Expose data quality in backend state for tabs that need it
  ...
  useEffect(() => {
    ...
    // Connect WebSocket for live rate streaming from backend
    connectWebSocket({
      onRates: (data) => {
    ...
    // Fetch forecast/cashflow/monte-carlo from backend on mount
    ...
    // Core tick interval (3 seconds)
    const tickInterval = setInterval(() => {
      // Use backend rates if available, otherwise generate locally
    ...
    // Periodic cashflow refresh (every 1 hour)
    ...
    // Periodic Monte Carlo refresh (every 30 minutes)
  ...
  const handleExecuteDeployment = useCallback(async ({ plan, surplus }) => {
    try {
      ...
      return { ok: false, message: `Deployment failed: ${err.message || "Unknown error"}` };
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-0)", position: "relative" }}>
      ...
      <Header backend={backend} time={time} alertCount={alertCount} rateItems={rateItems} backendConnected={backendConnected} dataQuality={dataQuality} forecastSource={forecastSource} />
