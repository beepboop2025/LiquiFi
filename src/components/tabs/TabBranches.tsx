...
import { DEMO_BRANCHES, DEMO_REGIONAL_SUMMARY, REGIONS } from "../../constants/branches";
import { fetchBranches, fetchBranchDetail, fetchBranchesSummary } from "../../services/api";
import SectionTitle from "../shared/SectionTitle";
import StatBox from "../shared/StatBox";
import type { Branch, RegionalSummary, RegionConfig } from "../../types";

interface TabBranchesProps {
  backendConnected?: boolean;
}

interface BranchHistoryPoint {
  date: string;
  cash: number;
  deployed: number;
  pnl: number;
...
interface BranchesSummaryResponse {
  regions?: Record<string, RegionalSummary & { branch_count?: number }>;
  totals?: BankTotals | null;
}

export default function TabBranches(_props: TabBranchesProps) {
  const [branches, setBranches] = useState<Branch[]>(DEMO_BRANCHES);
  const [regionSummary, setRegionSummary] = useState<Record<string, RegionalSummary & { branch_count?: number }>>(DEMO_REGIONAL_SUMMARY);
  const [selectedBranch, setSelectedBranch] = useState<string | null>(null);
  const [branchHistory, setBranchHistory] = useState<BranchHistoryPoint[]>([]);
  const [totals, setTotals] = useState<BankTotals | null>(null);

  useEffect(() => {
    fetchBranches().then((d: Branch[] | null) => { if (d) setBranches(d); });
    fetchBranchesSummary().then((d: BranchesSummaryResponse | null) => {
      if (d) {
  ...
  const handleSelectBranch = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (detail) setBranchHistory(detail as BranchHistoryPoint[]);
    } catch (err) {
  ...
  const bankTotals: BankTotals = totals || {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 18 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {Object.entries(regionSummary).map(([region, data]) => (
            <div key={region} style={{ background: "var(--bg-1)", borderRadius: 8, padding: 14, borderLeft: `3px solid ${(REGIONS as Record<string, RegionConfig>)[region]?.color || "var(--text-3)"}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: (REGIONS as Record<string, RegionConfig>)[region]?.color || "var(--text-1)" }}>{region}</span>
        ...
        <div className="card">
          ...
          <div className="scrollbar-thin" style={{ maxHeight: 400, overflowY: "auto" }}>
            {branches.map((b) => {
              const pos = b.position || ({} as Record<string, number>);
              ...
              return (
                <div key={b.code}>
                  ...
                  >
                    ...
                    <span className="badge mono" style={{ background: `${(REGIONS as Record<string, RegionConfig>)[b.region]?.color || "var(--text-3)"}18`, color: (REGIONS as Record<string, RegionConfig>)[b.region]?.color || "var(--text-3)", justifySelf: "start" }}>{b.region}</span>
                      ...
                      {branchHistory.length > 0 ? (
                        <ResponsiveContainer width="100%" height={180}>
                          <AreaChart data={branchHistory}>
                            ...
                            <XAxis dataKey="date" tick={{ fontSize: 9, fill: "var(--text-4)" }} interval={4} tickFormatter={(v: string) => v?.split("-").slice(1).join("/")} />
