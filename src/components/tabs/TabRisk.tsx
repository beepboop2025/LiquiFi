...
// Helper to safely access LCR/NSFR values which may come as snake_case from backend
...
// Helper for ALM bucket fields that may come as snake_case
            ...
            return (
              <div key={i} style={{ background: "var(--bg-1)", borderRadius: 8, padding: 12, textAlign: "center", borderTop: `2px solid ${d.color}` }}>
                ...
                <div className="progress-bar" style={{ marginTop: 6 }}>
                  <div className="progress-fill" style={{ width: `${Math.min(Number(usage), 100)}%`, background: Number(usage) > 85 ? "var(--red)" : Number(usage) > 70 ? "var(--amber)" : d.color }} />
                </div>
                <div className="mono" style={{ fontSize: 9, color: Number(usage) > 85 ? "var(--red)" : "var(--text-4)", marginTop: 4 }}>{usage}% used</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="card">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={almChartData}>
            ...
            <Tooltip contentStyle={{ background: "var(--bg-1)", border: "1px solid var(--border-2)", fontSize: 11 }} formatter={(v: number) => [`₹${v.toLocaleString()} Cr`]} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div style={{ background: "var(--bg-1)", borderRadius: 8, padding: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                ...
                <span className="mono" style={{ fontSize: 18, fontWeight: 700, color: getLcrPct(lcrNsfr.lcr as Record<string, unknown>) >= 100 ? "var(--green)" : "var(--red)" }}>
                  {getLcrPct(lcrNsfr.lcr as Record<string, unknown>).toFixed(1)}%
                </span>
              </div>
              <div className="progress-bar" style={{ height: 4 }}>
                <div className="progress-fill" style={{ width: `${Math.min(getLcrPct(lcrNsfr.lcr as Record<string, unknown>), 150) / 1.5}%`, background: getLcrPct(lcrNsfr.lcr as Record<string, unknown>) >= 100 ? "var(--green)" : "var(--red)" }} />
              </div>
            ...
            <div style={{ background: "var(--bg-1)", borderRadius: 8, padding: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                ...
                <span className="mono" style={{ fontSize: 18, fontWeight: 700, color: getNsfrPct(lcrNsfr.nsfr as Record<string, unknown>) >= 100 ? "var(--green)" : "var(--red)" }}>
                  {getNsfrPct(lcrNsfr.nsfr as Record<string, unknown>).toFixed(1)}%
                </span>
              </div>
              <div className="progress-bar" style={{ height: 4 }}>
                <div className="progress-fill" style={{ width: `${Math.min(getNsfrPct(lcrNsfr.nsfr as Record<string, unknown>), 150) / 1.5}%`, background: getNsfrPct(lcrNsfr.nsfr as Record<string, unknown>) >= 100 ? "var(--green)" : "var(--red)" }} />
              </div>
