      ...
      <div className="rate-ticker-inner">
        {doubled.map((r, i) => (
          <div key={`${r.name}-${i}`} style={{ display: "flex", alignItems: "center", gap: 8, whiteSpace: "nowrap" }}>
            ...
            <span className="mono" style={{ color: "var(--text-0)", fontSize: 12, fontWeight: 600 }}>{(r.rate ?? 0).toFixed(2)}{r.isFx ? "" : "%"}</span>
            <span className="mono" style={{ fontSize: 9, color: (r.ch ?? 0) >= 0 ? "var(--green)" : "var(--red)", display: "flex", alignItems: "center" }}>
