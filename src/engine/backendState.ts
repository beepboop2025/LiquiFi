import { rand, randIntInclusive } from '../utils/math';
import { createId, randomOf } from '../utils/helpers';
import { ORDER_BOOK, PAYMENT_STREAMS } from '../constants/erp';
import { INTEGRATION_BLUEPRINT } from '../constants/integrations';
import type {
  AuditEntry,
  PaymentStream,
  BackendState,
 ...
 */
export const createRealtimePayment = (): PaymentStream => ({
  id: createId("TXN"),
  type: randomOf(["RTGS", "NEFT", "UPI", "IMPS"] as const)! as PaymentStream["type"],
  ...
  status: randomOf(["settled", "pending", "processing", "settled", "settled", "failed"] as const)! as PaymentStream["status"],
 ...
 */
export const createBackendState = (): BackendState => ({
  killSwitch: false,
  failoverMode: "auto",
  circuitOpen: false,
  ...
  integrations: INTEGRATION_BLUEPRINT.map((sys: Integration) => ({ ...sys })),
